...
import { createMigrateToSqliteCommand } from '../src/app/cli/commands/migrateToSqliteCommand.js';
...
test('migrate-to-sqlite phase 2 imports valid ledger lines and skips malformed lines', async () => {
  ...
  const result = await command({ s3OutputPrefix: 'output' }, storage, { category: 'mouse', phase: '2' });
