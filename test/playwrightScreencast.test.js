...
test('PlaywrightFetcher accepts onScreencastFrame in constructor options', async () => {
  const { PlaywrightFetcher } = await import('../src/fetcher/playwrightFetcher.js');

  const frames = [];
  const onScreencastFrame = (frame) => frames.push(frame);
  const config = loadConfig({ preferHttpFetcher: true });
  const fetcher = new PlaywrightFetcher(config, null, { onScreencastFrame });

  assert.equal(typeof fetcher.onScreencastFrame, 'function');
});

test('PlaywrightFetcher has no screencast overhead when callback absent', async () => {
  const { PlaywrightFetcher } = await import('../src/fetcher/playwrightFetcher.js');

  const config = loadConfig({ preferHttpFetcher: true });
  const fetcher = new PlaywrightFetcher(config, null);

  assert.equal(fetcher.onScreencastFrame, undefined);
});

test('PlaywrightFetcher receives worker_id from source in screencast frame', async () => {
  const { PlaywrightFetcher } = await import('../src/fetcher/playwrightFetcher.js');
  ...
  assert.equal(frames[0].data, 'fake-jpeg-data');
});
