...
import {
  ...
  setupFixture,
} from './runtimeOpsRoutesHarness.js';
...
test('runtimeOpsRoutes: inactive run with stale running meta resolves to failed terminal summary', async () => {
  const { tempRoot, indexLabRoot, outputRoot } = await createRuntimeOpsRoot('runtime-ops-terminal-state-');
...
test('runtimeOpsRoutes: relocated local run remains readable after source indexlab directory is removed', async () => {
  const { tempRoot, indexLabRoot, outputRoot } = await createRuntimeOpsRoot('runtime-ops-relocated-');
  ...
  await fs.writeFile(
    path.join(archivedIndexLabRoot, 'run.json'),
    `${JSON.stringify({
      run_id: runId,
      category,
