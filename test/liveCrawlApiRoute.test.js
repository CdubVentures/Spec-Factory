...
import { registerIndexlabRoutes } from '../src/features/indexing/api/indexlabRoutes.js';
...
function createMockCtx(overrides = {}) {
  ...
  return {
    ctx: {
      ...
      buildEffectiveSettingsSnapshot: (config) => ({ ts: '2026-03-09T12:00:00.000Z', searchProvider: 'searxng' }),
      ...overrides
    },
    responses
  };
...
test('GET /indexlab/live-crawl/check-catalog returns catalog metadata', async () => {
  ...
  assert.equal(responses[0].body.total_checks, 148);
...
test('GET /indexlab/live-crawl/settings-snapshot returns snapshot', async () => {
  ...
  assert.equal(responses[0].body.searchProvider, 'searxng');
});
