...
test('studio map/docs writes are owned by studio persistence authority', () => {
  ...
  assert.equal(authorityText.includes('/save-drafts'), false, 'studio persistence authority should not use deprecated save-drafts route usage');
  ...
  assert.equal(studioPageText.includes('const saveEditsMut = useMutation('), false, 'Studio page should not own docs save mutation');
