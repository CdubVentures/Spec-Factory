...
export function Phase05Panel({
  ...
  return (
    <div className="sf-surface-panel p-3 space-y-3" style={{ order: 49 }}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center text-sm font-semibold sf-text-primary">
          ...
          <span>Parallel Fetch & Parse</span>
      ...
      {!collapsed ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-5 xl:grid-cols-10 gap-2 sf-text-caption">
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">in-flight now<Tip text="Current number of fetch jobs that are running right now." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.activeInflight)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">peak in-flight<Tip text="Highest concurrent in-flight fetch count seen during this run view." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.peakInflight)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">fetch started<Tip text="Total fetch-start events emitted so far." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.started)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">fetch completed<Tip text="Fetch jobs that completed successfully regardless of downstream parse/index state." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.completed)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">fetch failed<Tip text="Fetch jobs that ended in an explicit failure event." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.failed)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">http finished<Tip text="Completed fetches using HTTP transport." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.httpCount)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">browser finished<Tip text="Completed fetches using browser automation/rendering path." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.browserCount)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">other/unknown<Tip text="Completed fetches where transport mode is missing or non-standard." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.otherCount)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">p95 fetch<Tip text="95th percentile fetch duration (network + transfer), from fetch completion events." /></div>
              <div className="font-semibold">{formatLatencyMs(phase5Runtime.fetchP95Ms)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">p95 parse<Tip text="95th percentile parse/extraction duration captured on processed sources." /></div>
              <div className="font-semibold">{formatLatencyMs(phase5Runtime.parseP95Ms)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">article sampled<Tip text="Parse events that carried article extraction telemetry (method/quality/chars)." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.articleSamples)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">readability hits<Tip text="Count of parse events where article extractor method was Readability." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.articleReadability)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">fallback hits<Tip text="Count of parse events where fallback/heuristic article extraction was used." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.articleFallback)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">avg article score<Tip text="Average article quality score from extraction telemetry (0-100)." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.articleAvgScore, 1)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">low-quality article<Tip text="Article extraction rows flagged as low quality by score/length guard." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.articleLowQuality)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">json-ld rows<Tip text="Phase 05 structured metadata rows detected in JSON-LD blocks across processed URLs." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredJsonLd)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">microdata rows<Tip text="Phase 05 structured metadata rows detected in Microdata surfaces." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredMicrodata)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">opengraph keys<Tip text="Phase 05 OpenGraph key count from structured sidecar extraction." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredOpengraph)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">structured cands<Tip text="Structured candidates accepted by identity gate and forwarded into deterministic extraction lane." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredCandidates)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">structured rejected<Tip text="Structured candidates rejected by target identity gate (multi-product or mismatch)." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredRejected)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">structured errors<Tip text="Sidecar extraction errors captured fail-open; parsing continues without blocking run." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.structuredErrors)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf docs parsed<Tip text="Phase 06 text-PDF docs successfully parsed from manufacturer/manual links." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfDocsParsed)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf pairs<Tip text="Total normalized PDF key/value assertions extracted (kv + table)." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfPairsTotal)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf kv pairs<Tip text="Line/key-value style assertions extracted from text PDF surfaces." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfKvPairs)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf table pairs<Tip text="Table-grid assertions extracted from structured PDF tables." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfTablePairs)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf pages scanned<Tip text="Total PDF pages scanned by parser backends for processed docs." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfPagesScanned)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">pdf errors<Tip text="PDF parser/router errors emitted while keeping runtime fail-open." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.pdfErrors)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">scanned docs<Tip text="Phase 07 scanned/image-only PDFs detected during parse routing." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfDocsDetected)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr attempted<Tip text="Scanned PDFs where OCR worker path was attempted." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfOcrDocsAttempted)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr succeeded<Tip text="Scanned PDFs that produced at least one OCR key/value pair." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfOcrDocsSucceeded)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr pairs<Tip text="Total OCR-derived pairs from scanned PDF surfaces (kv + table)." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfOcrPairs)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr kv/table<Tip text="OCR pair split by key-value and table-derived rows." /></div>
              <div className="font-semibold">
                {formatNumber(phase5Runtime.scannedPdfOcrKvPairs)}/{formatNumber(phase5Runtime.scannedPdfOcrTablePairs)}
              </div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr low conf<Tip text="OCR pairs flagged below confidence threshold for caution/review." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfOcrLowConfPairs)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr conf avg<Tip text="Average OCR confidence reported across scanned PDF OCR attempts." /></div>
              <div className="font-semibold">{formatNumber(phase5Runtime.scannedPdfOcrConfidenceAvg, 3)}</div>
            </div>
            <div className="sf-surface-elevated px-2 py-1">
              <div className="sf-text-subtle flex items-center">ocr errors<Tip text="Scanned PDF OCR errors captured fail-open while extraction continues." /></div>
            ...
            <table className="min-w-full sf-text-caption">
              ...
              <tbody>
                {phase5Runtime.hostsActive.length === 0 ? (
                  <tr>
                    <td className="py-2 sf-text-subtle" colSpan={2}>no active hosts</td>
                  </tr>
                ) : (
                  phase5Runtime.hostsActive.map((row) => (
                    <tr key={`phase5-host:${row.host}`} className="sf-table-row border-b sf-border-soft">
          ...
          <div className="sf-surface-elevated p-2 space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2 sf-text-caption">
              <div className="sf-surface-elevated px-2 py-1">
                <div className="sf-text-subtle flex items-center">fallback started<Tip text="Fetcher mode fallback attempts initiated after a fetch error (403, timeout, 5xx, network error)." /></div>
                <div className="font-semibold">{formatNumber(phase5Runtime.schedulerFallbackStarted)}</div>
              </div>
              <div className="sf-surface-elevated px-2 py-1">
                <div className="sf-text-subtle flex items-center">fallback ok<Tip text="Fallback attempts that succeeded with an alternate fetcher mode." /></div>
                <div className="font-semibold sf-status-text-success">{formatNumber(phase5Runtime.schedulerFallbackSucceeded)}</div>
              </div>
              <div className="sf-surface-elevated px-2 py-1">
                <div className="sf-text-subtle flex items-center">fallback exhausted<Tip text="URLs where all fetcher modes were tried and none succeeded. The URL is marked as failed." /></div>
                <div className="font-semibold sf-status-text-danger">{formatNumber(phase5Runtime.schedulerFallbackExhausted)}</div>
              </div>
              <div className="sf-surface-elevated px-2 py-1">
                <div className="sf-text-subtle flex items-center">host waits<Tip text="Times the scheduler paused before fetching to respect per-host minimum delay (prevents rate-limiting)." /></div>
                <div className="font-semibold">{formatNumber(phase5Runtime.schedulerHostWaits)}</div>
              </div>
              <div className="sf-surface-elevated px-2 py-1">
                <div className="sf-text-subtle flex items-center">ticks<Tip text="Scheduler processing ticks (one per completed fetch slot). Higher values = more concurrent throughput." /></div>
                <div className="font-semibold">{formatNumber(phase5Runtime.schedulerTicks)}</div>
              </div>
            </div>
            <div className="mt-1 sf-table-shell">
            <table className="min-w-full sf-text-caption">
              <thead className="sf-table-head border-b sf-border-soft">
                <tr>
              ...
              <tbody>
                {phase5Runtime.schedulerFallbackFeed.length === 0 ? (
                  <tr>
                    <td className="py-2 sf-text-subtle" colSpan={7}>no scheduler fallback events yet</td>
                  </tr>
                ) : (
                  phase5Runtime.schedulerFallbackFeed.map((row, idx) => (
                    <tr key={`sched-fb:${row.ts}:${row.kind}:${idx}`} className="sf-table-row border-b sf-border-soft">
                      <td className="py-1 pr-3">{formatDateTime(row.ts)}</td>
                      <td className={`py-1 pr-3 font-semibold ${
                        row.kind === 'succeeded' ? 'sf-status-text-success'
                          : row.kind === 'exhausted' ? 'sf-status-text-danger'
                          : ''
                      }`}>{row.kind}</td>
          ...
          <div className="sf-surface-elevated p-2 overflow-x-auto">
            ...
            <div className="mt-1 sf-text-label sf-text-subtle">
            ...
            {indexlabDynamicFetchDashboardResp?.key ? (
              <div className="mt-1 sf-text-label sf-text-subtle font-mono truncate" title={String(indexlabDynamicFetchDashboardResp.key || '')}>
            ...
            <table className="min-w-full sf-text-caption">
              ...
              <tbody>
                {dynamicFetchDashboardHosts.length === 0 ? (
                  <tr>
                    <td className="py-2 sf-text-subtle" colSpan={10}>
                      no dynamic dashboard rows yet
                    </td>
                  </tr>
                ) : (
                  dynamicFetchDashboardHosts.slice(0, 24).map((row) => (
                    <tr key={`dyn-host:${row.host || 'unknown'}`} className="sf-table-row border-b sf-border-soft">
                      <td className="py-1 pr-3 font-mono">{row.host || '-'}</td>
            ...
            <table className="min-w-full sf-text-caption">
              ...
              <tbody>
                {phase5Runtime.structuredSnippetRows.length === 0 ? (
                  <tr>
                    <td className="py-2 sf-text-subtle" colSpan={5}>no structured snippet rows yet</td>
                  </tr>
                ) : (
                  phase5Runtime.structuredSnippetRows.slice(0, 24).map((row, idx) => (
                    <tr key={`structured-snippet:${row.ts}:${row.key_path}:${idx}`} className="sf-table-row border-b sf-border-soft">
            ...
            <table className="min-w-full sf-text-caption">
              ...
              <tbody>
                {phase5ArticleDomainLeaderboard.length === 0 ? (
                  <tr>
                    <td className="py-2 sf-text-subtle" colSpan={9}>no article-domain rows yet</td>
                  </tr>
                ) : (
                  phase5ArticleDomainLeaderboard.map((row) => (
                    <tr key={`article-host:${row.host}`} className="sf-table-row border-b sf-border-soft">
                      <td className="py-1 pr-3 font-mono">{row.host}</td>
            ...
            {phase5ArticlePreviewJob ? (
              <>
                ...
                <div className="sf-surface-elevated p-2 text-xs">
                  <div className="sf-text-subtle mb-1">title</div>
                  <div className="font-medium sf-text-primary">
                ...
                <pre className="sf-surface-elevated p-2 text-xs whitespace-pre-wrap break-words max-h-56 overflow-auto">
