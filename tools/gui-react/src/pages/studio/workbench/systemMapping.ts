...
//   1. Add the field path to FIELD_SYSTEM_MAP with all consuming systems.
//   2. Add a CONSUMER_TOOLTIPS entry for every system listed in the map.
//   3. Run: node --test test/systemMappingCoverage.test.js
//
// When an existing field gains a new consumer:
//   1. Add the system to the field's FIELD_SYSTEM_MAP array.
//   2. Add the corresponding CONSUMER_TOOLTIPS { on, off } entry.
...
export const FIELD_SYSTEM_MAP: Record<string, SystemSet> = {
  ...
  // Priority
  'priority.required_level':        SEED_IDX_REV,
  'priority.availability':          SEED_IDX_REV,
  'priority.difficulty':            SEED_IDX_REV,
  'priority.effort':                SEED_IDX_REV,
  'priority.publish_gate':          IDX_REV,
  'priority.block_publish_when_unk': IDX_REV,

  // AI Assist
  'ai_assist.mode':             SEED_IDX_REV,
  'ai_assist.model_strategy':   IDX_REV,
  'ai_assist.max_calls':        IDX_REV,
  'ai_assist.max_tokens':       IDX_REV,
  'ai_assist.reasoning_note':   IDX_REV,

  // Parse
  'parse.template':             IDX_REV,
  'parse.unit':                 IDX_REV,
  'parse.unit_accepts':         IDX_REV,
  ...
  // Evidence
  'evidence.required':           SEED_IDX_REV,
  'evidence.min_evidence_refs':  SEED_IDX_REV,
  'evidence.conflict_policy':    IDX_REV,
  'evidence.tier_preference':    IDX_REV,
...
// ── Per-field, per-system tooltip descriptions ──────────────────────────
// Each entry describes what happens when a system is ON vs OFF for a field.
// This is the single source of truth — all 3 tabs read from this map.
...
export const CONSUMER_TOOLTIPS: Record<string, Partial<Record<DownstreamSystem, ConsumerTip>>> = {
  ...
  'contract.unknown_token': {
    indexlab: {
      on: 'IndexLab uses this token (e.g., "unk", "N/A") as the placeholder when no value can be extracted from any source.',
      off: 'IndexLab uses the global default unknown token instead of the field-specific one.',
    },
  },
  'contract.rounding.decimals': {
    indexlab: {
      on: 'IndexLab rounds numeric values to this many decimal places after extraction (e.g., 2 → "3.14").',
      off: 'IndexLab keeps full precision. Numeric values may have inconsistent decimal places across products.',
    },
  },
  'contract.rounding.mode': {
    indexlab: {
      on: 'IndexLab applies this rounding mode (nearest, floor, ceil) when rounding numeric values.',
      off: 'IndexLab defaults to "nearest" rounding regardless of this setting.',
    },
  },
  ...
  'priority.publish_gate': {
    indexlab: {
      on: 'IndexLab blocks publishing if this field has no value. The product cannot be exported with this field empty.',
      off: 'IndexLab allows publishing even when this field is empty. Missing data will not block the export.',
    },
    review: {
      on: 'LLM Review marks products as unpublishable when this field is unresolved. Shows a publish-blocking warning.',
      off: 'LLM Review does not block publishing based on this field. Empty values are accepted.',
    },
  },
  ...
  'ai_assist.max_calls': {
    indexlab: {
      on: 'IndexLab limits LLM API calls for this field to the configured maximum. Controls cost and extraction depth per field.',
      off: 'IndexLab uses the default call budget derived from the effort level. No per-field call limit applied.',
    },
    review: {
      on: 'LLM Review respects this per-field call budget during AI validation. Limits how many validation passes are attempted.',
      off: 'LLM Review uses the default call budget from effort level. No per-field cap on validation calls.',
    },
  },
  ...
  'parse.unit': {
    indexlab: {
      on: 'IndexLab expects extracted values in this unit and applies unit-aware parsing (e.g., strips "grams" suffix, normalizes to "g").',
      off: 'IndexLab does not apply unit-aware parsing. Values are extracted as-is without unit recognition.',
    },
    review: {
      on: 'LLM Review validates that candidate values use the correct unit. Unit mismatches are flagged during validation.',
      off: 'LLM Review does not check unit correctness. Candidates with wrong units pass through undetected.',
    },
  },
  'parse.unit_accepts': {
    indexlab: {
      on: 'IndexLab accepts these alternate unit spellings (e.g., "grams", "g", "gram") during parsing and normalizes them to the canonical unit.',
      off: 'IndexLab only accepts the exact canonical unit. Values with alternate unit spellings are rejected as unparseable.',
    },
    review: {
      on: 'LLM Review recognizes these alternate unit spellings during validation. Candidates using accepted variants are not flagged as unit errors.',
      off: 'LLM Review only accepts the exact canonical unit during validation. Alternate spellings are treated as mismatches.',
    },
  },
  'parse.allow_unitless': {
    indexlab: {
      on: 'IndexLab accepts numeric values without an explicit unit suffix (e.g., "120" without "g"). Useful when units are implied by context.',
      off: 'IndexLab rejects numeric values that do not include an explicit unit suffix. Unitless numbers are treated as unparseable.',
    },
  },
  'parse.allow_ranges': {
    indexlab: {
      on: 'IndexLab accepts range values (e.g., "100-200g") and parses both lower and upper bounds as structured data.',
      off: 'IndexLab rejects range values. Only single numbers are accepted — ranges are treated as unparseable.',
    },
  },
  'parse.strict_unit_required': {
    indexlab: {
      on: 'IndexLab requires a unit to be present on every numeric value. Unitless numbers are always rejected, overriding allow_unitless.',
      off: 'IndexLab respects the allow_unitless setting for unit flexibility.',
    },
  },
