...
export const FINDER_SETTINGS_REGISTRY: Record<FinderIdWithSettings, readonly FinderSettingsEntry[]> = {
  ...
  'keyFinder': [
    ...
    { key: 'budgetVariantPointsPerExtra', type: 'float', default: 0.25, min: 0, max: 10, uiLabel: 'Variant points per extra', uiTip: 'Points added to the per-key attempt budget for each variant beyond the first. Fractional so variant count is a gentle accrual, not a whole-point punch per variant. Raw budget accrues the fractional value; final attempts = ceil(raw). Does NOT scale bundling passenger cost — passenger cost is always the raw bundlingPassengerCost value.', uiGroup: 'Budget Scoring' },
    ...
    { key: 'bundlingPassengerCost', type: 'intMap', default: {"easy":1,"medium":2,"hard":4,"very_hard":8}, min: 0, max: 64, keys: ['easy', 'medium', 'hard', 'very_hard'] as const, keyLabels: {"easy":"Easy","medium":"Medium","hard":"Hard","very_hard":"Very hard"}, uiLabel: 'Passenger cost', uiTip: 'Point cost to carry a passenger of each difficulty. Cost is raw (not scaled by variant count) — easy pool / easy cost = max easy passengers.', uiGroup: 'Bundling' },
    ...
    { key: 'passengerExcludeAtConfidence', type: 'int', default: 0, min: 0, max: 100, uiLabel: 'Exclude passengers at confidence ≥', uiTip: '"Good enough" exclusion. When > 0 AND Min evidence > 0, peers whose top candidate confidence is at or above this threshold AND meets the evidence minimum are dropped from the passenger pool. Below either threshold, peers keep retrying. 0 = disabled (only published peers are dropped).', uiGroup: 'Bundling' },
    { key: 'passengerExcludeMinEvidence', type: 'int', default: 0, min: 0, max: 50, uiLabel: 'Exclude passengers min evidence', uiTip: 'Companion to "Exclude passengers at confidence ≥". Evidence count (substantive, excluding identity_only refs) that a peer’s top candidate must reach alongside the confidence threshold to be excluded. Both knobs must be > 0 for the exclusion rule to engage.', uiGroup: 'Bundling' },
