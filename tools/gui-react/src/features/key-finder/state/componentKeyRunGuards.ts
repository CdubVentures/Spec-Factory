...
export function isKeyRunBlocked(row: ComponentKeyRunState): boolean {
  ...
  return isComponentIdentityChildKey(row) && row.component_dependency_satisfied === false;
}

export function componentRunBlockTitle(parentKey: string | undefined): string {
  return `Run ${parentKey || 'the parent component'} first. Component brand/link are locked until the parent component has a published value.`;
}
