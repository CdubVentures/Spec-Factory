...
const options: DropdownModelOption[] = [
  ...
  { value: 'default-deepseek:deepseek-chat', label: 'deepseek-chat', providerId: 'default-deepseek', providerName: 'DeepSeek' },
];
...
describe('buildDropdownItems isDefault badging contracts', () => {
  ...
  it('moves the default badge when globalDefaultModelId changes', () => {
    ...
    const second = buildDropdownItems({
      options, allowNone: false, noneLabel: '(none)', globalDefaultModelId: 'deepseek-chat', missingOption: null,
    });
    strictEqual(first.find((i) => i.isDefault)?.value, 'default-gemini:gemini-2.5-flash');
    strictEqual(second.find((i) => i.isDefault)?.value, 'default-deepseek:deepseek-chat');
  });

  it('matches an exact composite-key globalDefaultModelId as well as a bare id', () => {
    const items = buildDropdownItems({
      options, allowNone: false, noneLabel: '(none)', globalDefaultModelId: 'default-deepseek:deepseek-chat', missingOption: null,
    });
    const defaults = items.filter((i) => i.isDefault);
    strictEqual(defaults.length, 1);
    strictEqual(defaults[0].value, 'default-deepseek:deepseek-chat');
  });
