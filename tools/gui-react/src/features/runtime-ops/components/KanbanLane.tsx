...
export function KanbanCard({ title, domain, snippet, score, rationale, titlePrefix, children, onClick }: KanbanCardProps) {
  return (
    <div
      className={`border sf-border-default rounded p-2 sf-surface-elevated text-xs ${onClick ? 'cursor-pointer sf-card-hover-accent' : ''}`}
      onClick={onClick}
    >
      <div className="font-medium sf-text-primary truncate flex items-center">{titlePrefix}{title}</div>
