...
const CALL_TYPE_META: Record<string, CallTypeDef> = {
  ...
  escalation_planner:   { symbol: '\u21D1', label: 'Escalation',       prefetchTab: null, tabCode: null,  chipClass: 'sf-chip-warning',     barClass: 'sf-bar-ct-serp-triage' },
...
interface LlmCallsDashboardProps {
  runId: string;
  category: string;
  isRunning: boolean;
...
export function LlmCallsDashboard({
  runId,
  category,
  isRunning,
  highlightWorkerId,
  ...
  const { data: dashboard } = useQuery({
    ...
    queryFn: () => api.get<LlmCallsDashboardResponse>(`/indexlab/run/${runId}/runtime/llm-dashboard`),
