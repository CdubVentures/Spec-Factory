...
import {
  computeSerpSelectorDecisionCounts,
  computeSerpSelectorUniqueDomains,
  buildSerpSelectorDecisionSegments,
  buildSerpSelectorFunnelBullets,
} from '../../selectors/serpSelectorHelpers.js';
import { PrefetchEmptyState } from './PrefetchEmptyState.tsx';
import type { RuntimeIdxBadge } from '../../types.ts';

interface PrefetchSerpSelectorPanelProps {
...
function CandidateDrawer({
  ...
  candidate: SerpSelectorCandidate;
    ...
    <DrawerShell title={candidate.title || candidate.url} subtitle={candidate.domain} onClose={onClose}>
      ...
      <DrawerSection title="Score Decomposition">
        ...
        <div className="grid grid-cols-2 gap-1 mt-2 text-xs">
          ...
          <span><Chip label={candidate.decision} className={serpSelectorDecisionBadgeClass(candidate.decision)} /></span>
        </div>
      </DrawerSection>
...
export function PrefetchSerpSelectorPanel({ calls, serpSelector, persistScope, idxRuntime }: PrefetchSerpSelectorPanelProps) {
  ...
  const [expandedQueries, toggleExpandedQuery, replaceExpandedQueries] = usePersistedExpandMap(
    `runtimeOps:prefetch:serpSelector:expandedQueries:${persistScope}`,
  );
  const [selectedCandidateKey, setSelectedCandidateKey] = usePersistedNullableTab<string>(
    `runtimeOps:prefetch:serpSelector:selectedCandidate:${persistScope}`,
    null,
  ...
  const counts = useMemo(() => computeSerpSelectorDecisionCounts(triage), [triage]);
  const uniqueDomains = useMemo(() => computeSerpSelectorUniqueDomains(triage), [triage]);
  const decisionSegments = useMemo(() => buildSerpSelectorDecisionSegments(counts), [counts]);
  const funnelBullets = useMemo(() => buildSerpSelectorFunnelBullets(triage, calls), [triage, calls]);
  const hasDecisions = counts.keep + counts.dropped_by_llm + counts.hard_drop > 0;
          ...
          <div className="sf-surface-elevated rounded-sm border sf-border-soft px-5 py-4">
            <div className="flex flex-col gap-1.5">
              {[
                ...
                { label: 'Sent to LLM', count: funnel.candidates_sent_to_llm, delta: null, tip: 'All classified candidates sent to LLM for evaluation' },
        ...
        <div>
          ...
          <div className="space-y-2">
            {triage.map((t, ti) => {
              ...
              const droppedByLlm = allCandidates.filter((c) => c.decision !== 'keep' && c.decision !== 'hard_drop');
                          ...
                          {allCandidates.map((c, ci) => (
                            ...
                            >
                              ...
                              <td className="py-1.5 px-4">
                                <Chip label={c.decision} className={serpSelectorDecisionBadgeClass(c.decision)} />
                              </td>
                              <td className="py-1.5 px-4">
