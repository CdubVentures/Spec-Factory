...
implementation/
  ...
  ai-indexing-plans/                        - canonical Section 03 plans and critical path
    ...
    indexlab-full-pipeline.mmd              - source-of-truth diagram
    indexlab-full-pipeline.svg/.png         - rendered outputs
    parsing-managament/
      PARSING-01-06-core-surfaces.md
      PARSING-07-11-advanced-surfaces.md
      12-14 schema json files
...
We are working on `SECTION-03-ai-indexing-lab-execution.md` using the consolidated canonical plan files in `implementation/ai-indexing-plans/`.

### Canonical Plan Structure (2026-02-24)

- `PHASES-00-03-foundation-and-discovery.md`
- `PHASES-04-06-core-ingestion-and-evidence.md`
- `PHASES-07-09-retrieval-extraction-convergence.md`
- `PHASES-10-13-learning-ops-and-batch.md`
- `parsing-managament/PARSING-01-06-core-surfaces.md`
- `parsing-managament/PARSING-07-11-advanced-surfaces.md`
- `CRITICAL-PATH-REMAINING-IMPLEMENTATION.md` (single backlog source)

### Current Audit Baseline (2026-02-24)

- Full suite baseline: `npm test` -> 2726 pass, 1 fail.
- Current deterministic failing test: `test/reviewLaneContractGui.test.js` (category visibility timeout in GUI flow).
- Focused audit suites pass:
  - `test/phase03SerpTriage.test.js`
  - `test/indexingDomainChecklistApi.test.js`
  - `test/noDeadConfig.test.js`
  - `test/automationWorker.test.js`
  - `test/ftsRetrieval.test.js`
  - `test/learningTwoProductProof.test.js`

### Remaining Work Is Tracked Only In

- `implementation/ai-indexing-plans/CRITICAL-PATH-REMAINING-IMPLEMENTATION.md`

### Critical Path (Execution Order)

1. Fix deterministic GUI lane contract failure (`test/reviewLaneContractGui.test.js`).
2. Wire Phase 04 repair signals into durable Phase 06B queue/worker end-to-end.
3. Complete Phase 08B visual manifest pipeline with quality and target gates.
4. Implement Parsing 08 image OCR and wire `image_asset_refs` into Phase 08 context.
5. Complete parallel deferred items (Parsing 07/09/10, Phase 11 knob telemetry, Phase 01 timestamp lineage).
...
New modules: 12 route/helper files (guiServer), 10 pipeline helper files (runProduct), 21 panel/type/helper files (IndexingPage), 7 barrel exports.
...
src/api/routes/                        - 12+ route handler modules (extracted)
src/api/routes/runtimeOpsDataBuilders.js - 11 pure data builder functions (Phase 13)
src/api/routes/runtimeOpsRoutes.js     - Runtime Ops REST endpoints (Phase 13)
...
tools/gui-react/src/pages/runtime-ops/RuntimeOpsPage.tsx - Runtime Ops page shell (275 LOC)
tools/gui-react/src/pages/runtime-ops/panels/        - 13 panel components (Phase 13)
```

### Event Model (NDJSON - Phase 00)
Events are written to `artifacts/indexlab/<run_id>/run_events.ndjson` and pushed via WebSocket.
All events include `worker_id` (Phase 13.3 fix): `fetch-N`, `search-N`, `llm-{batchId}`.
...
```
need = missing_multiplier x conf_term x required_weight
       x tier_deficit_multiplier x min_refs_deficit_multiplier x conflict_multiplier
```
...
Use only the 7 consolidated markdown files listed in `## Active Work: Section 03 - AI Indexing Lab Execution`.
There are no CSV trackers in this directory anymore.

Diagram authority:
- Source: `implementation/ai-indexing-plans/indexlab-full-pipeline.mmd`
- Rendered outputs: `indexlab-full-pipeline.svg` and `indexlab-full-pipeline.png` (4K)

Do NOT modify canonical files unless updating status or implementation notes.
Items are considered done only when code and GUI proof criteria are both satisfied.
