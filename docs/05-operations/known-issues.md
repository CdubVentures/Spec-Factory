...
| `POST /api/v1/test-mode/run` remains effectively non-functional because `runTestProduct` is stubbed in route context | test mode can create, generate, and validate categories, but the run step returns per-product error rows | use create/generate/validate flows only until the crawl-first runner is rebuilt | `src/app/api/routes/testModeRouteContext.js`, `src/app/api/routes/testModeRoutes.js` |
