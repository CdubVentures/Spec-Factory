...
| Intel Graph helper API | `src/app/api/routes/infra/graphqlRoutes.js`, `src/app/api/intelGraphApi.js` | local GraphQL server on `http://127.0.0.1:8787/graphql` | JSON GraphQL POST proxy | proxy returns `502 graphql_proxy_failed` when helper is absent or unhealthy |
...
| source | `src/app/api/intelGraphApi.js` | local GraphQL helper endpoint |
