...
These are persisted durably through `/api/v1/ui-settings` (saved in `helper_files/_runtime/user-settings.json`), with `localStorage` used as a local mirror for cross-session continuity.
Legacy `sessionStorage` autosave keys are read once and migrated into `localStorage`.
...
- LLM settings auto-save is global in `useUiStore` local state (`llmSettings:autoSaveEnabled`) and is synchronized to `/api/v1/ui-settings`.
