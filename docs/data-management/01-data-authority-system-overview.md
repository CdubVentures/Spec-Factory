...
1. Authoring sources (JSON + field-studio map + generated artifacts) define category data.
...
- `src/field-rules/sessionCache.js` (compiled+saved-map merge and cache)
- `src/ingest/categoryCompile.js` (field-studio map load/save/validate)
...
- `field-studio-map` save is blocked if `validate-map` fails (frontend and backend).
- `field-studio-map-saved` is emitted for field-studio-map writes and drives studio/mapping/review-layout invalidation.
- runtime/convergence/LLM settings writes are authority-owned by shared frontend hooks (not page-local route calls).
- Studio map-doc writes are authority-owned by `studioPersistenceAuthority.ts` while remaining category-scoped files.
