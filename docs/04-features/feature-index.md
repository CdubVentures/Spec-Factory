...
| Crawl | Persistent browser session, fetch-plan execution, plugin runner, and block-status classification. Pipeline-internal (no API routes; consumed by indexing and pipeline layers). | -- | `src/features/crawl/index.js`, `src/features/crawl/crawlSession.js`, `src/features/crawl/plugins/pluginRegistry.js`, `src/features/crawl/bypassStrategies.js` |
| Extraction | Screenshot, video, and HTML artifact capture and persistence during crawl runs. Pipeline-internal (no API routes; consumed by the pipeline layer). | -- | `src/features/extraction/index.js`, `src/features/extraction/core/extractionRunner.js`, `src/features/extraction/plugins/pluginRegistry.js` |
...
| Publisher | 12-phase validation pipeline, discovery enums (self-tightening vocabulary), repair adapter, and record orchestrator. Pipeline-internal (no API routes; consumed by review and indexing layers). | -- | `src/features/publisher/index.js`, `src/features/publisher/validation/validateField.js`, `src/features/publisher/validation/validateRecord.js`, `src/features/publisher/validation/phaseRegistry.js` |
...
| Review Curation (deprecation facade) | Re-exports the full review domain API from `src/features/review/`. Exists as a backward-compatibility barrel; no unique logic. | -- | `src/features/review-curation/index.js` |
...
- The table above lists all 13 verified backend features under `src/features/`. Four are pipeline-internal (crawl, extraction, publisher, review-curation) with no HTTP API routes.
