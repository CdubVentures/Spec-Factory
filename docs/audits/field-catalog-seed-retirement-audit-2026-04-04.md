...
| `src/features/expansion-hardening/expansionHardening.js` | REFACTOR | Output exposed a dead path. | Bootstrap rows now report meaningful created-file counts plus category/manifest paths. | Expansion hardening tests green; `npm test` green; retired symbol grep 0. | Updated |
| `src/features/expansion-hardening/tests/expansionHardening.test.js` | COLLAPSE | Previous assertion only proved dead seed path emission. | Contract now checks bootstrap row count and created-file counts for real scaffold outputs. | Expansion hardening tests green; `npm test` green. | Updated |
