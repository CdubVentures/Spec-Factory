...
| Synthetic test data | `src/testing/testDataProvider.js` | KEEP | Generated identities now split family vs variant correctly and stop duplicating variant into full-name strings. |
...
| Catalog/indexing/runtime helpers | `src/app/api/processRuntime.js`, `src/app/api/catalogHelpers.js`, `src/features/indexing/api/builders/processStartLaunchPlan.js`, `src/features/indexing/api/builders/runListBuilder.js`, `src/pipeline/seams/bootstrapRunProductExecutionState.js`, `src/features/indexing/pipeline/shared/queryPlan.js`, `src/features/color-edition/colorEditionLlmAdapter.js`, `src/ingest/csvIngestor.js` | KEEP | Runtime and data-path helpers preserve `base_model` through launch, status, CSV ingest, and display derivation. |
| Launcher/build root resolution | `src/core/nativeModuleGuard.js`, `src/api/guiServerRuntime.js`, `tools/build-exe.mjs` | KEEP | Bundle/runtime root resolution now uses module-relative paths instead of `process.cwd()` assumptions. |
...
| `src/features/expansion-hardening/tests/expansionHardening.test.js` | KEEP | Protects expansion runtime hardening path with real queue/spec-db dependencies. | None. | `P3`, `F1` | Kept; harness fixed instead of weakening assertions. |
...
| `src/testing/tests/testRunner.test.js` | KEEP | Protects synthetic execution/test harness identity wiring. | None. Strengthened in place. | `P3`, `P4`, `F1` | Kept as harness behavior coverage. |
| `src/testing/tests/testDataProviderSourceIdentity.test.js` | KEEP | Protects generated test-source identity splits and prevents variant duplication. | None. Strengthened in place. | `P3`, `P4`, `F1` | Kept as direct contract proof. |
...
- the `gui:exe` rebuild succeeded, but esbuild emitted `import.meta` warnings for `src/api/guiServerRuntime.js` and `src/core/nativeModuleGuard.js`
