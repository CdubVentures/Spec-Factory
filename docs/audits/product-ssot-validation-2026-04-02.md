...
| 3 (CRUD to SQL-only) | **Validated** | `addProduct`, `addProductsBulk`, `updateProduct`, `removeProduct`, `seedFromCatalog` all use `specDb` for dedup/existence. Temporary identity compatibility shims that existed during this phase were later retired in the 2026-04-04 contract audit. |
| 4 (Identity + queue to SQL) | **Validated** | `identityGate`, `productIdentityAuthority`, `fieldReviewHandlers`, `reconciler`, `queueState` all operate SQL-first. |
...
Previous failures (updateProduct regression + contract export) were fixed and verified green.
...
- Overlap 0a (product identity): `product.json` is sole disk SSOT. SQL is a derived cache. `loadProductCatalog` retired.
