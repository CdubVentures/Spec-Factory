...
├── _generated/
│   ├── component_db/
│   │   ├── sensors.json          # Compiled from field-studio source spreadsheet
│   │   ├── switches.json
│   │   ├── encoders.json
│   │   └── materials.json
...
```mermaid
flowchart LR
    A["_generated/component_db/<br/>sensors.json<br/><i>(field-studio baseline)</i>"] --> B["_overrides/components/<br/>sensor_*.json<br/><i>(user + AI overrides)</i>"]
...
flowchart TD
    ...
    subgraph "CellDrawer Shows"
        ...
        J["Field Studio candidate (from spreadsheet import)"]
...
| 4 | **`approved_new` doesn't auto-create component DB entry** | By design | When a human approves a new component, the item is marked `approved_new` but the actual component DB row must be added via the field-studio source spreadsheet or a future "Add Component" UI. This is a deliberate safety guardrail — new component rows affect schema and require human authoring. |
...
| 9 | **Maker identity observations not enriched** | Minor | `buildComponentReviewPayloads` enriches `name_tracked.candidates` with pipeline identity observations but does not do the same for `maker_tracked.candidates`. Only the field-studio value appears as a maker candidate. |
...
| `tools/gui-react/src/pages/studio/StudioPage.tsx` | Match/AI/Priority configuration sliders and dropdowns | Within Component & spreadsheet section |
...
|--------------|---------|
| `helper_files/{cat}/_generated/component_db/{type}.json` | Compiled component database (from spreadsheet) |
...
graph TB
    subgraph "1. Compile Time"
        XLSX["spreadsheet Field Studio"] -->|"categoryCompile.js"| FRULES["field_rules.json<br/>match: { fuzzy_threshold, property_weight, ... }<br/>ai: { mode: judge, ... }<br/>priority: { difficulty, effort }"]
