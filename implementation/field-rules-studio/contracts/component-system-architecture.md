...
This document is the implementation-facing contract for Field Studio rules, map capture, component matching, and review payload wiring.
...
- authoring surfaces in `field_studio_map.json` saved docs
...
- `_control_plane/field_studio_map.json`
...
- `loadCategoryConfig()` projects compiled rules and `sessionCache` map-doc overlays.
...
- Result: map-doc artifacts under `helper_files/{category}/_control_plane/*` are not merged into global `_runtime/settings.json` snapshots.

---

## 3. Field Studio Capture Contract
...
2. map `field_overrides` overlays from control-plane session cache
3. explicit map `selected_keys` field order overlay (group markers reconstructed as `__grp::`)
...
- mouse field-studio component source alignment fixed:
...
- selected keys map cleanly into field-studio field entries
...
- review route field-rule payload keeps compiled metadata plus saved-map/session overlays
