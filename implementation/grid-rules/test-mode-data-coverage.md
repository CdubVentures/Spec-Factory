...
|-----------|-------|------|------|
| `contractDriven.test.js` | 168 | 168 | 0 |
| `componentReviewDataLaneState.test.js` | 19 | 19 | 0 |
| `reviewGridData.test.js` | 11 | 11 | 0 |
...
|------|-----|---------------|
| `buildFieldState propagates constraint_conflict from constraint_analysis contradictions` | GAP-6 | `summary.constraint_analysis.contradictions` with `code: 'constraint_conflict'` → `reason_codes` includes `constraint_conflict` |
| `buildFieldState propagates compound_range_conflict from constraint_analysis contradictions` | GAP-6 | `code: 'compound_range_conflict'` → `reason_codes` includes `compound_range_conflict` (takes priority over `constraint_conflict`) |
| `buildFieldState does not apply contract.rounding.decimals — characterization` | GAP-8 | Value `67.456` passes through as-is — `contract.rounding` is NOT consumed at grid level |

### New tests in `contractDriven.test.js` (+2 tests + 3 inline assertions, 166→168)

| Test / Assertion | Gap | What it proves |
|-----------------|-----|---------------|
| `FLAG-05 — cross_validation product has constraint_conflict flag` | GAP-6 | E2e: `cross_validation` scenario seeds `constraint_analysis.contradictions` → product grid payload has `constraint_conflict` in `reason_codes` |
| `ENUM-03 — pipeline-suggested values have needs_review=true` | GAP-5 | Pipeline-suggested values for `open_prefer_known` catalogs arrive with `needs_review: true` + `source: 'pipeline'` |
| Section 4 GAP-4 assertion (inline) | GAP-4 | Pipeline-discovered component rows have `new_component` in `name_tracked.reason_codes` |
| Section 4 GAP-7 assertion (inline) | GAP-7 | At least one component property slot has `variance_violation` when `variance_policies` exist |

### Test data changes in `contractDriven.test.js`

| Change | Purpose |
|--------|---------|
| `sourceResultsToArtifacts` now conditionally adds `constraint_analysis.contradictions` for `cross_validation` scenario | Seeds the constraint data that `buildFieldState` reads to produce `constraint_conflict` flags |
| `_suggestions/enums.json` seeded with `NewTestValue_{catalog}` for `open_prefer_known` catalogs | Provides pipeline suggestions so `buildEnumReviewPayloadsLegacy` can produce `needs_review: true` entries |

### Gap status summary

| Gap | Status | Evidence |
|-----|--------|----------|
| GAP-1 (`object×object` runtime) | Deferred | `object_schema` is compile-only by design |
| GAP-2 (`selection_policy.source_field` runtime) | Deferred | Post-consensus reducer niche path |
| GAP-4 (`new_component` → grid) | Closed | Section 4 inline assertion |
| GAP-5 (`new_enum_value` → grid) | Closed | ENUM-03 test |
| GAP-6 (`constraint_conflict` → grid) | Closed | Unit test + FLAG-05 e2e |
| GAP-7 (`variance_violation` → grid) | Closed | Section 4 inline assertion |
| GAP-8 (rounding enforcement) | Closed | Characterization test proves dead config at grid level |
| GAP-9 (`dependency_missing` never fires) | Already covered | FLAG-01 happy_path 0 flags |
