...
|---|---|---|---|---|
| Authoring control plane | Field Studio map docs | `helper_files/{category}/_control_plane/field_studio_map.json` | `PUT /studio/{category}/field-studio-map` | Studio mapping queries, `sessionCache`, compile |
| Authoring control plane | Product catalog | `helper_files/{category}/_control_plane/product_catalog.json` | catalog routes, compile utilities | Studio catalog products, seed |
...
- Studio map docs (category scoped):
  - writer: `tools/gui-react/src/pages/studio/studioPersistenceAuthority.ts`
  - route: `PUT /api/v1/studio/{category}/field-studio-map`
...
2. Saved map `field_overrides` override baseline via `sessionCache` deep merge.
3. Saved map `selected_keys` order overrides compiled order; group markers (`__grp::`) are reconstructed from per-field group metadata.
4. Authority snapshot token is derived from map timestamp, compiled timestamp, and SpecDb sync version.
...
- `GET /api/v1/studio/{category}/payload`
  - merged `fieldRules` + merged `fieldOrder` + compile/map timestamps
- `GET /api/v1/studio/{category}/field-studio-map`
  - normalized field-studio map
