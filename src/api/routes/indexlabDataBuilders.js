...
function buildNeedSetFromEvents(meta = {}, eventRows = []) {
  for (let i = eventRows.length - 1; i >= 0; i -= 1) {
    ...
    return {
      ...
      pending_fields: Math.max(0, toInt(payload?.pending_fields, needs.length)),
      unresolved_fields: Math.max(0, toInt(payload?.unresolved_fields, needs.length)),
...
function pickSearchQueryFromEvent(row = {}) {
  const payload = row?.payload && typeof row.payload === 'object'
    ? row.payload
    : {};
  const direct = String(
...
function buildSearchProfileFromEvents(meta = {}, eventRows = []) {
...
export async function readIndexLabRunNeedSet(runId) {
  ...
  const resolvedRunId = String(meta?.run_id || token).trim();
  const productId = resolveRunProductId(meta, eventRows);
  if (category && resolvedRunId && productId && _storage && typeof _storage.resolveOutputKey === 'function') {
...
export async function readIndexLabRunSearchProfile(runId) {
  ...
  if (category && resolvedRunId && _storage && typeof _storage.resolveInputKey === 'function' && typeof _storage.readJsonOrNull === 'function') {
