    ...
    try {
      ...
      if (predictableSources.length > 0) {
        ...
        if (predictedUrls.length > 0) {
          ...
          logger?.info?.('urls_predicted', {
            remaining_budget: Math.max(0, discoveryCap - rawResults.length),
            predictions: predictedUrls.slice(0, 30).map((p) => ({
              url: String(p?.url || '').trim(),
              domain: String(p?.source_host || '').trim(),
              predicted_payoff: p?.predicted_tier ? Math.max(0, 100 - (p.predicted_tier - 1) * 25) : 50,
              target_fields: [],
              risk_flags: [],
              decision: 'fetch'
            }))
