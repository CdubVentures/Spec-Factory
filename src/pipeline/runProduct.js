...
export async function runProduct({
  ...
  const screencastCallback = config.runtimeScreencastEnabled && typeof config.onScreencastFrame === 'function'
  ...
  let fetcher = initialFetcherMode === 'dryrun'
    ...
    : initialFetcherMode === 'http'
      ...
      : initialFetcherMode === 'crawlee'
        ...
        : new PlaywrightFetcher(config, logger, { onScreencastFrame: screencastCallback });
  ...
  const processPlannerQueue = async () => {
    ...
    const fetchSourcePageData = async ({ source, sourceHost, hostBudgetRow } = {}) => {
      fetchWorkerSeq += 1;
      source.worker_id = `fetch-${fetchWorkerSeq}`;
