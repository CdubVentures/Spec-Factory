...
- `settingsContract.js`: document metadata constants, runtime/convergence/UI/storage key sets and value types, route-contract exports, `SETTINGS_AUTHORITY_PRECEDENCE`, `UI_SETTINGS_DEFAULTS`, and `validateUserSettingsSnapshot()`.
...
- `runtimeSettingsRouteContract.js` and `convergenceSettingsRouteContract.js`: canonical runtime and convergence route-contract constants.
...
## Domain Invariants
- `user-settings.json` is the canonical persisted document for runtime, convergence, storage, UI, and studio sections.
