...
export const KEY_FINDER_VARIABLES = Object.freeze([
  ...
  { name: 'VARIANT_INVENTORY', description: 'Locked active CEF variant table joined to SKU/RDF/PIF by variant_id. Rendered only when useful variant identity exists and the key enables variant inventory context. Includes edition/SKU/release/colorway guardrails and forbids Key Finder from extracting colors, editions, sku, or release_date.', required: false, category: 'deterministic' },
  { name: 'FIELD_IDENTITY_USAGE', description: 'Per-primary-field instructions for how to use {{VARIANT_INVENTORY}} as an evidence filter. Generated deterministically when ai_assist.variant_inventory_usage.enabled is not false. Enable only when variant identity adds value without ambiguity; field-specific union/exact/base/default interpretation belongs in ai_assist.reasoning_note.', required: false, category: 'deterministic' },
