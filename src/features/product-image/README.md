...
- **Variant deletion cascades to PIF**: When a variant is deleted, `propagateVariantDelete` removes all images, evaluations, and carousel slots referencing that variant_id/variant_key. SQL projection is updated. Image files on disk are NOT deleted (de-indexed only).
