    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ...
          ['Requiredness is coverage strategy', '`mandatory` if a normal buyer can find this in public evidence (spec sheet, manufacturer page, credible review, canonical product render) for the typical product in the category. `non_mandatory` if the value typically requires lab measurement, product disassembly, or proprietary internal-component identity work. Grade for the typical product, not the flagship.'],
          ['Difficulty is routing', '`easy` = first SERP for the obvious query OR visible in any product render. `medium` = page 2, refined query, OR a specific angle/spec-table section. `hard` = multiple queries + sites + light cross-analysis; answer still in public text. `very_hard` = same effort PLUS deduction across signals OR lab-only/instrumented measurements OR proprietary internal-component identities behind unmarked silicon.'],
          ['Unknown / no / n/a', '`no` is a proven negative for boolean fields, `n/a` is an intentional not-applicable value only when the contract wants it, and unknown means no submitted value plus an unknown reason. Do not add `unk` as an enum value.'],
    ...
    blocks: [
      {
        ...
        rows: [
          ...
          ['Unknown handling', '`yes`, `no`, `n/a`, blank, omitted, and unknown statuses are distinct where the contract needs them.'],
