...
function buildPerKeyBlocks(key, adapterPreview) {
  ...
  if (!key.component) {
    ...
    blocks.push({
      kind: 'paragraph',
      text: 'Classify this key from scratch as component identity, component attribute, identity facet, or standalone. Decide semantic ownership by invariance first; boolean, date, range, url, and list-shaped fields can still be component attributes when the value belongs to the component. Do not wait for an existing component DB property before proposing a component attribute. If it is an attribute, choose the parent component and add the field to patch.component_sources roles.properties[]. If it is an identity, use enum.source = component_db.<component_type>. Auto-generated identity facets <component_type>_brand and <component_type>_link must stay out of component_sources.<component_type>.roles.properties[]; record them as identity_facet in the roster audit.',
    });
  }
