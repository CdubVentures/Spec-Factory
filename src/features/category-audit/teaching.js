...
16. **Standard facet shape for new component identities.** When proposing a NEW component identity for a category, also propose the standard facet field rules: \`<key>_brand\` (string scalar with \`component_identity_projection: { component_type: <key>, facet: 'brand' }\`) and \`<key>_link\` (url scalar with the same projection and \`facet: 'link'\`). Optionally \`<key>_type\` for categorical type/technology classification when meaningful. These facet rules are authored independently — they are not auto-generated when the identity is created. Without them, the brand and link semantics for the component aren't expressible.
17. **Component property invariance test (the gate).** A component property must be an INVARIANT of the component itself — the same value for every product that uses this component. Anything that varies per device, per firmware, per PCB layout, per host MCU or integration, per testing methodology, or per calibration is NEVER a component property; it is a product field. Universal test the auditor applies before promoting a key into \`component_sources[<X>].roles.properties[]\`: "Does this value change across two different products that use the same component?" If yes → leave the key standalone (product field). If no → candidate component property. Three universal failure modes (apply across every tech category):
