    ...
    "component_sources": [
      {
        ...
        "roles": {
          "properties": [
            ...
            {
              "field_key": "<boolean_component_fact>",
              "type": "boolean",
              "variance_policy": "authoritative"
            },
            {
...
- \`data_lists\` rows use only \`field\` and \`manual_values\`. Do not put priority, AI guidance, source metadata, normalize settings, delimiters, or parsing columns on enum lists.
- \`component_sources\` rows use only \`component_type\` and \`roles.properties[]\`. Do not put priority, AI guidance, entity names, makers, aliases, links, source URLs, notes, or import metadata on component source rows.
...
- Auto-generated identity facets \`<component_type>_brand\` and \`<component_type>_link\` must not be listed under \`component_sources.<component_type>.roles.properties[]\`; record them as \`identity_facet\` in \`audit.adjacent_key_roster_decisions[]\`.
- \`audit.adjacent_key_roster_decisions[]\` records every adjacent-key structural decision for component identity audits.
- \`audit.schema_blocked_component_attributes[]\` records semantic component attributes that cannot be encoded directly in \`patch.component_sources\` yet; include \`field_key\`, \`component_type\`, \`expected_type\`, \`expected_shape\`, \`variance_policy\`, and \`reason\`.
...
- For component identity keys, the component DB is already the lookup/lock path. Use \`open_prefer_known\` by default with \`enum.source = "component_db.<same_key>"\` so known rows/aliases are preferred and evidence-backed new entities can be marked for curation. Use \`closed\` only for genuinely finite/static component sets where every new entity must be pre-curated before extraction.
- \`<component_type>_brand\` and \`<component_type>_link\` are auto-generated identity facets when the component identity exists; do not add them to \`patch.component_sources[].roles.properties[]\`. Keep them out of the property roster and record them as \`identity_facet\` in the audit roster decisions.
- Decide component membership by semantic ownership and invariance first, not by the current primitive type. Boolean, date, url, range, mixed_number_range, and list-shaped fields can be component attributes when the value is invariant for the component. If semantic ownership is component-backed but the patch schema cannot encode the needed shape/type, leave the unsupported edit out of \`patch.component_sources\` and record it under \`audit.schema_blocked_component_attributes[]\`.
- For component identity audits, scan name-prefixed and conceptually adjacent keys (brand, link, type, date, latency, settings, boolean feature flags, mode-specific siblings). Record each as \`component_property\`, \`component_only_property\`, \`identity_facet\`, \`standalone\`, or \`schema_blocked\` in \`audit.adjacent_key_roster_decisions[]\`; identity audits own the full roster.
...
- Enum Data Lists belong under \`patch.data_lists\`. Use only \`field\` and \`manual_values\`; return the final ordered canonical values when replacing a list; keep aliases/source phrases out of public chips. List-level priority, normalize settings, and AI guidance are retired; key priority and extraction guidance stay under \`field_overrides.<field_key>\`.

Key Navigator guidance:
- Field contract, priority, evidence, enum policy, constraints, field-name aliases for standalone/product fields, search_hints, and ai_assist belong under \`patch.field_overrides.<field_key>\`.
...
15. **Sibling-alias discipline.** When a field has connection/mode/variant-qualified siblings in the same family (e.g. parent \`<key>\` plus \`<key>_wired\` / \`<key>_wireless\` / \`<key>_<mode>\`, or \`<key>\` plus \`<key>_peak\` / \`<key>_sustained\`, or \`<key>\` plus \`<key>_xmp\` / \`<key>_jedec\`), the parent's aliases must include only mode-agnostic phrases. Any source phrase carrying a qualifier (\`wired\`, \`wireless\`, \`peak\`, \`sustained\`, \`xmp\`, \`overclocked\`, \`burst\`, etc.) belongs to that specific sibling, not the parent. Before locking aliases, scan the category key map for sibling fields whose name extends the audited key with a qualifier suffix; for each, exclude qualifier-bearing phrases from the parent's aliases and place them on the sibling instead.
16. **Auto-generated facet shape for component identities.** When a component identity exists in \`component_sources\`, the compiler automatically materializes \`<key>_brand\` (string scalar with \`component_identity_projection.facet=brand\`) and \`<key>_link\` (url scalar with \`facet=link\`). Do not add those keys to \`component_sources.<key>.roles.properties[]\`; they are identity facets, not component properties. If their guidance needs tuning, audit the facet key itself in \`field_overrides.<key>_brand\` or \`field_overrides.<key>_link\`. Optional non-standard facets such as \`<key>_type\` still require an explicit field rule decision.
17. **Component-identity enum.policy default — evolving entity sets pick \`open_prefer_known\`, not \`closed\`.** The component DB is already the lookup/lock path: \`enum.source = component_db.<key>\` routes extraction through component rows and aliases. When a component-identity field's entity set evolves over time (new entities released regularly — sensors, switches, panels, GPU chips, RAM modules, etc.), default to \`open_prefer_known\`. The runtime accepts new entities found in evidence with \`mark_needs_curation=true\` for human review, so the publisher gate never blocks a legitimate new product on day one. Reserve \`closed\` for genuinely finite/static sets where every new entity MUST be a deliberate human-curation gate. Do not pick \`closed\` because the current entity list happens to be enumerable today or because "component DB" sounds locked; pick policy by whether the entity set is *expected to grow*. Universal: applies to every component family across every category.

18. **Component-identity and auto-facet aliases discipline.** When \`enum.source = component_db.<X>\`, do not author \`field_overrides.<component>.aliases\`; leave it blank/absent. The same rule applies to auto-generated identity facets such as \`field_overrides.<component>_brand.aliases\` and \`field_overrides.<component>_link.aliases\`. Component row aliases are component data, not Field Studio patch data.

19. **Component property invariance test (the gate).** A component property must be an INVARIANT of the component itself — the same value for every product that uses this component. Anything that varies per device, per firmware, per PCB layout, per host MCU or integration, per testing methodology, or per calibration is NEVER a component property; it is a product field. Universal test the auditor applies before promoting a key into \`component_sources[<X>].roles.properties[]\`: "Does this value change across two different products that use the same component?" If yes → leave the key standalone (product field). If no → candidate component property. Apply semantic ownership before type: dates, booleans, URLs, ranges, and list-shaped fields can all be component properties when invariant. Three universal failure modes (apply across every tech category):
    - **Lab/measurement results** — measured input/click/response latency, effective clock under load, measured power draw, calibration-dependent color accuracy, instrumented thermal performance, polling-rate-dependent timing, measured THD/FR, per-sample silicon variance.
    - **Firmware/MCU-implemented feature toggles** — features whose presence depends on the host product's firmware/MCU/integration, even when the underlying component is the same. Examples across categories: motion-sync algorithms, software smoothing, vendor latency-reduction integrations (e.g. NVIDIA Reflex), proprietary haptic/signal-processing features, product-side overclock or boost toggles, vendor-specific noise-cancellation modes.
    - **Product-exposed configuration / capability subsets** — products subset or cap the component's native capability through firmware/MCU choices. Examples across categories: product-exposed polling-rate ladders, product-exposed lift settings, product-exposed DPI step list, product-side rate or refresh caps, exposed XMP/JEDEC profile menus.

    Universal examples of values that PASS the invariance test (legitimate component properties): spec capabilities marketed by the maker (sensor max DPI, panel native refresh, chip rated boost clock, switch marketed actuation force, RAM module rated speed, driver diaphragm size), and lineage-fixed properties (the component's own release date, marketed technology class, marketed lineage qualifier). Name signals (not authoritative — always apply the invariance test): \`*_latency\`, \`*_latency_*\`, \`*_response_time\`, \`measured_*\`, \`*_under_load\`, \`effective_*\`, \`*_at_<condition>\` are almost always per-device measurements; \`*_settings\` and feature-name booleans implemented in firmware are usually per-product feature toggles; \`*_date\`, \`*_brand\`, \`*_type\`, \`*_link\`, marketed-cap/marketed-rating fields are usually invariants.
`.trim();
...
The priority triple is a search/routing contract. Use a "human Googler" yardstick — what does it take a normal buyer to verify this value for the typical product in the category, not the best-documented flagship?

- **\`required_level\`** — \`mandatory\` if a normal buyer can find this in public evidence (spec sheet, manufacturer page, credible review, canonical product render, or stated component identity) for the typical product. \`non_mandatory\` if the value typically requires lab measurement, product disassembly, or proprietary internal-component identity work that public sources rarely expose. Mandatory + unk blocks publish; non_mandatory + unk resolves quietly.
- **\`availability\`** — \`always\` if credible public sources expose this for nearly every product. \`sometimes\` if coverage is uneven (flagships have it, budget/older/boutique brands often don't). \`rare\` if only specialist sources (labs, teardowns, niche reviews) publish this for a small fraction of the category.
- **\`difficulty\`** — \`easy\` = first SERP for the obvious query OR visible in any product render. \`medium\` = page 2, refined query, OR a specific angle/spec-table section is needed. \`hard\` = multiple queries + sites + light cross-analysis; the answer still exists in public text once found. \`very_hard\` = same effort PLUS deduction across signals (component lineage, indirect inference) OR lab-only/instrumented measurements OR proprietary internal-component identities behind unmarked silicon.
...
- \`type\` â€” \`"string"\`, \`"number"\`, \`"integer"\`, \`"boolean"\`, \`"date"\`, \`"url"\`, \`"range"\`, or \`"mixed_number_range"\`. Drives engine matching behavior. Choose component ownership by invariance first; type never disqualifies a component attribute.
...
- \`component_only\` â€” boolean. When \`true\`, the property is tracked in the component DB but does NOT publish as a product field and does not require a matching \`field_overrides\` entry. Use for component-scoped depth content (for example, \`sensor_native_resolution_steps\` on a sensor entity). The JSON patch may declare the mapping; missing/stale concrete entity values stay outside the Field Studio patch.
...
**Numeric-only collapse.** \`upper_bound\`, \`lower_bound\`, \`range\` are meaningful only for numeric properties (\`number\`, \`integer\`, \`range\`, or \`mixed_number_range\`). For non-numeric properties such as \`string\`, \`boolean\`, \`date\`, or \`url\`, the engine auto-collapses to \`authoritative\` regardless of authored value.
...
**Canonical-form anchor for the parent identity field.** The canonical-form anchor for a component identity is the \`component_db.<X>\` table, NOT \`enum.match.format_hint\`. Component identity value shapes are too irregular within a category — maker part numbers, branded product lines, generation suffixes, distributor codes, host-branded variants — to capture in 1–4 patterns. Any regex tight enough to reject slugified marketing also rejects legitimate new entities at the publisher gate. Discipline lives in three places: (a) the curated component_db entities + per-entity aliases, (b) \`enum.policy: open_prefer_known\` so new entities found in evidence get curated rather than blocked, (c) the parent identity's \`reasoning_note\` carrying the anti-pattern bank, identity-layer choice, marketing-vs-spec rules, and host-brand-leak guard. Do not author \`enum.match.format_hint\` on a component-identity field.
...
**Roster completeness scan.** Before locking the \`component_sources[<identity>].roles.properties[]\` roster, scan the full category key list rendered in the per-key brief (and in Part 7 of the audit report) for any keys that are name-prefixed with this component (\`<identity>_*\`) or conceptually adjacent (latency / date / link / generation / sub-feature variants). For each such key, decide structurally whether it should join the property roster (as a product-backed property or a \`component_only\` depth attribute), become a facet via \`component_identity_projection\`, stay standalone, or be reported under \`schema_blocked_component_attributes\` if the semantic component attribute cannot be encoded by the current patch shape. Identity audits own this structural decision; do not leave name-adjacent keys unconsidered.
