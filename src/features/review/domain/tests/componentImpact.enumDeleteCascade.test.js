...
import { unpublishEnumValueFromProducts } from '../componentImpact.js';

function createHarness() {
  const root = path.join('.tmp', `_test_enum_delete_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`);
...
test('unpublishEnumValueFromProducts removes published field and preserves candidates plus evidence', async () => {
  const harness = createHarness();
  try {
    harness.specDb.insertFieldCandidate({
      productId: 'mouse-delete',
      fieldKey: 'connection',
    ...
    const candidate = harness.specDb.getFieldCandidateBySourceId('mouse-delete', 'connection', 'src-connection-1');
    ...
    harness.writeProduct('mouse-delete', {
      fields: {
        connection: {
          value: 'wired',
          confidence: 0.95,
    ...
    const result = await unpublishEnumValueFromProducts({
      productRoot: harness.root,
      category: 'mouse',
      field: 'connection',
      value: 'wired',
      productIds: ['mouse-delete'],
      specDb: harness.specDb,
    });

    const product = harness.readProduct('mouse-delete');
    const row = harness.specDb.getFieldCandidateBySourceId('mouse-delete', 'connection', 'src-connection-1');

    assert.equal(result.unpublished, 1);
    assert.equal(product.fields.connection, undefined);
