...
test('DB SEED - SpecDb table verification', async (t) => {
  await withSeededSpecDbFixture(async ({ db, config, seedResult }) => {
    ...
    await t.test('DB-01: all 6 tables have non-zero counts', () => {
      const counts = seedResult.counts;
      for (const table of ['component_values', 'component_identity', 'component_aliases', 'list_values', 'item_component_links', 'item_list_links']) {
