...
export function makeEnumRouteHarness(overrides = {}) {
  ...
  const context = {
    ...
    removeEnumValueFromDurableSources: () => ({ changed: false, sources: [] }),
    renameEnumValueInDurableSources: () => ({ changed: false, sources: [] }),
    renameEnumValueInProducts: async () => ({ affected: [], renamed: 0 }),
