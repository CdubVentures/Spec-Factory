 ...
 *
 * Caller owns product.json file I/O — this function mutates productJson.fields[fieldKey] in place.
 *
 * @param {{ specDb, productId: string, fieldKey: string, config: object, productJson: object }} opts
 * @returns {{ status: 'republished'|'unpublished'|'unchanged' }}
 */
export function republishField({ specDb, productId, fieldKey, config, productJson }) {
  if (!productJson.fields?.[fieldKey]) {
    return { status: 'unchanged' };
  }

  const remaining = specDb.getFieldCandidatesByProductAndField(productId, fieldKey);
  ...
  if (itemUnion === 'set_union') {
    ...
    specDb.markFieldCandidateResolved(productId, fieldKey, winner.value);
