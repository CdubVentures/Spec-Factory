...
export const bootstrapPhase = {
  ...
  async execute(ctx) {
    ...
    if (ctx.planner?.updateBrandHints) {
      ctx.planner.updateBrandHints(brandResolution);
      if (ctx.planner.seedManufacturerDeepUrls) {
        ctx.planner.seedManufacturerDeepUrls();
      }
    }
