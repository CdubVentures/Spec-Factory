...
import { runNeedSet } from '../needSet/runNeedSet.js';
import { runBrandResolver } from '../brandResolver/runBrandResolver.js';
import { runSearchProfile } from '../searchProfile/runSearchProfile.js';
import { runSearchPlanner } from '../searchPlanner/runSearchPlanner.js';
...
export async function runDiscoverySeedPlan({
  ...
  // === NeedSet and Brand Resolver (parallel) ===
  // WHY: These two phases are completely independent — neither needs the
  ...
  // Domain Classifier only reads from discoveryResult (candidates, selectedUrls) — safe to merge after.
  const finalResult = {
    ...discoveryResult,
