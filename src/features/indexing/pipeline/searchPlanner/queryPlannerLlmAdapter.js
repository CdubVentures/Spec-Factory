...
export function buildEnhancerSystemPrompt(rowCount) {
  return [
    ...
    '',
    'HISTORY AWARENESS (highest priority — overrides TIER rules below):',
    '- `query_history` lists queries already executed on prior runs. Producing a query that matches one in `query_history` (case-insensitive, punctuation-agnostic) is a failure.',
    '- `url_history` lists URLs already crawled on prior runs. Avoid queries that are obviously going to re-surface those exact URLs; prefer different phrasing families, different domain biases, or different content-type hints.',
    '- If a TIER 1 seed input query appears in `query_history`, you MUST produce a materially different variation — append a different phrasing family (teardown, benchmark, measured, review, spec sheet, comparison, reference), swap the domain bias for an untried one, or reorder/rephrase — the "return unchanged" TIER 1 rule does NOT apply in that case.',
    '',
    'TIER 1 — "seed": Broad product seed queries (e.g. "{brand} {model} specifications").',
    '- If the input query does NOT appear in query_history: return it unchanged or with only trivial phrasing cleanup. Do NOT restructure, add fields, or change intent.',
    '- If the input query DOES appear in query_history: HISTORY AWARENESS wins — produce a materially different variation as described above.',
    '',
