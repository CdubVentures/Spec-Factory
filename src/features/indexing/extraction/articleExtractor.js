...
const DEFAULT_MAX_READABILITY_HTML_CHARS = 500_000;
...
export function extractMainArticle(html, options = {}) {
  ...
  const maxReadabilityHtmlChars = Math.max(
    100_000,
    Number(options.maxReadabilityHtmlChars || DEFAULT_MAX_READABILITY_HTML_CHARS)
  );
  ...
  if (html.length > maxReadabilityHtmlChars) {
    return {
      method: 'heuristic_fallback',
      text: truncateForTokenBudget(fallback.text, maxChars),
      title: pageTitle || fallback.title,
      ...
      quality: fallbackQuality,
      low_quality: fallbackQuality.score < minScore || fallbackQuality.char_count < minChars,
      fallback_reason: 'html_too_large_for_readability'
    };
  }
