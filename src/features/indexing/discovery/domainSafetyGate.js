export async function classifyDomains({
  ...
  const uncached = [];
  const getCachedClassification = typeof storage?.getDomainClassification === 'function'
    ? (domain) => storage.getDomainClassification(domain)
    : () => null;
  const persistClassification = typeof storage?.upsertDomainClassification === 'function'
    ? (row) => storage.upsertDomainClassification(row)
    : () => {};

  for (const domain of toArray(domains)) {
    const key = normalizeDomain(domain);
    if (!key) continue;
    const cached = getCachedClassification(key);
    if (cached) {
      results.set(key, {
