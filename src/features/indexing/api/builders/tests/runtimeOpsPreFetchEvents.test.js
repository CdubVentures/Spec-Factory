...
test('serp_selector_completed events populate serp_selector array', () => {
  const events = [
  ...
  const result = buildPreFetchPhases(events, makeMeta(), {});
  assert.ok(Array.isArray(result.serp_selector), 'serp_selector should be an array');
  assert.equal(result.serp_selector.length, 1);
  assert.equal(result.serp_selector[0].query, 'Razer Viper V3 Pro specs');
  assert.equal(result.serp_selector[0].kept_count, 5);
  assert.equal(result.serp_selector[0].dropped_count, 3);
  assert.equal(result.serp_selector[0].candidates.length, 2);
  assert.equal(result.serp_selector[0].candidates[0].score, 0.95);
  assert.equal(result.serp_selector[0].candidates[0].decision, 'keep');
  assert.ok(result.serp_selector[0].candidates[0].score_components);
  assert.equal(result.serp_selector[0].candidates[0].score_components.base_relevance, 0.8);
});

test('serp_selector defaults to empty array when no events', () => {
  const result = buildPreFetchPhases([], makeMeta(), {});
  assert.ok(Array.isArray(result.serp_selector));
  assert.equal(result.serp_selector.length, 0);
});
...
test('new structured fields coexist with existing fields in buildPreFetchPhases', () => {
  ...
  assert.ok(Array.isArray(result.serp_selector), 'new serp_selector present');
...
test('serp_selector_completed handles candidates with missing score_components', () => {
  const events = [
    makeEvent('serp_selector_completed', {
      query: 'test query',
      kept_count: 1,
  ...
  const result = buildPreFetchPhases(events, makeMeta(), {});
  assert.equal(result.serp_selector[0].candidates[0].score, 0.7);
  assert.ok(result.serp_selector[0].candidates[0].score_components != null);
});
