...
test('buildPreFetchPhases: empty events returns baseline shape', () => {
  ...
  assert.deepEqual(result.llm_calls.serp_selector, []);
...
test('buildPreFetchPhases: groups triage/rerank/serp LLM calls under serp_selector', () => {
  const events = [
    makeEvent('llm_started', { reason: 'serp_selector_batch', batch_id: 'st-1' }),
    makeEvent('llm_finished', { reason: 'serp_selector_batch', batch_id: 'st-1' }),
  ...
  const result = buildPreFetchPhases(events, makeMeta(), {});
  assert.equal(result.llm_calls.serp_selector.length, 3);
});
...
test('buildPreFetchPhases: non-prefetch LLM calls are not included in any group', () => {
  ...
  assert.equal(result.llm_calls.serp_selector.length, 0);
