...
- **Candidate confidence is derived, not hardcoded**: The `confidence` passed to `submitCandidate` for both `colors` and `editions` is `max(evidence_refs.confidence)` across the discovery response (0 when no refs). This is how the publisher's `publishConfidenceThreshold` gate sees honest evidence strength instead of the previous hardcoded `100`. A CEF run with only weak-tier refs can now legitimately fail the publisher gate.
