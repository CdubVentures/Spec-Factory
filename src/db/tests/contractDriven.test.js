...
function sourceResultsToArtifacts(sourceResults, product, contractAnalysis) {
  ...
  const summary = {
    ...
    constraint_analysis: product._testCase?.name === 'cross_validation'
      ? {
        contradictions: (raw.rules || []).map(r => ({
          fields: [r.trigger_field, ...(r.related_fields || []), ...(r.depends_on || [])].filter(Boolean),
          code: 'constraint_conflict',
          severity: 'error',
          rule_id: r.rule_id || 'test',
        })),
      }
      : {},
  };
...
test('Contract-Driven End-to-End Test', async (t) => {
  ...
  try {
    ...
    const enumSuggestions = openPKCatalogs
      .filter(c => c.usingFields?.[0])
      .map(c => ({
        field_key: c.usingFields[0],
        value: `NewTestValue_${c.catalog}`,
    ...
    const itemComponentLinksByProduct = new Map(products.map((product) => [
      product.productId,
      db.getItemComponentLinks(product.productId),
    ]));
    const componentIdentityRowsByType = new Map(contractAnalysis._raw.componentTypes.map((ct) => [
      ct.type,
      db.getAllComponentIdentities(ct.type),
    ]));
    const getProductByScenarioId = (scenarioId) => productByScenarioId.get(scenarioId);
    const getProductByScenarioName = (scenarioName) => productByScenarioName.get(scenarioName);
    const getCandidateRowsForField = (productId, fieldKey) =>
      candidateGroupsByProduct.get(productId)?.[fieldKey] || [];
    const getItemFieldState = (productId, fieldKey) =>
      itemFieldStateByProductAndField.get(productId)?.get(fieldKey) || null;
    ...
    async function getReviewPayload(productId, { withSpecDb = false } = {}) {
      if (!withSpecDb) {
        return reviewPayloads.get(productId);
      }
      if (reviewPayloadsWithSpecDb.has(productId)) {
        return reviewPayloadsWithSpecDb.get(productId);
      }
    ...
    for (const matrixRow of contractAnalysis.matrices.fieldRules.rows) {
      ...
      for (const testId of matrixRow.testNumbers) {
        ...
        const product = getProductByScenarioId(testId);
        if (!product) continue;
        ...
        const payload = reviewPayloads.get(product.productId);
        ...
        if (minRefs > 1) {
          const dbCandidates = getCandidateRowsForField(product.productId, fieldKey);
        ...
        const dbFieldState = getItemFieldState(product.productId, fieldKey);
    ...
    for (const matrixRow of contractAnalysis.matrices.listsEnums.rows) {
      ...
      for (const testId of matrixRow.testNumbers) {
        ...
        const dbFieldState = catalogField
          ? getItemFieldState(product.productId, catalogField)
          : null;
        const dbValue = dbFieldState?.value != null ? String(dbFieldState.value) : '';
        const effectiveOutput = dbValue || outputStr;
    ...
    await t.test('SECTION 1 — Seed & DB Verification', async (s) => {
      ...
      await s.test('SEED-02 — component_identity has >=6 items per type and no exact duplicate name+maker rows', () => {
        for (const ct of contractAnalysis._raw.componentTypes) {
          const rows = componentIdentityRowsByType.get(ct.type) || [];
      ...
      await s.test('SEED-02b — maker-capable component types include A/B/makerless lanes with >=2 linked products each', () => {
        ...
        for (const ct of contractAnalysis._raw.componentTypes) {
          ...
          const identityRows = componentIdentityRowsByType.get(ct.type) || [];
      ...
      await s.test('SEED-03 — component_aliases findable by canonical name + aliases', () => {
        for (const ct of contractAnalysis._raw.componentTypes) {
          const identities = componentIdentityRowsByType.get(ct.type) || [];
      ...
      await s.test('SEED-06 — item_component_links created for exact-match products', () => {
        ...
        const varianceProduct = getProductByScenarioName('variance_policies');
        if (varianceProduct) {
          const links = itemComponentLinksByProduct.get(varianceProduct.productId) || [];
      ...
      await s.test('SEED-07 — candidates have evidence fields', () => {
        const happyProduct = getProductByScenarioName('happy_path');
        const candidateGroups = candidateGroupsByProduct.get(happyProduct.productId);
      ...
      await s.test('SEED-09 — shared components: multiple products link to same items', () => {
        ...
        for (const product of products) {
          const links = itemComponentLinksByProduct.get(product.productId) || [];
    ...
    await t.test('SECTION 2 — Per-Scenario Behavioral Verification', async (s) => {
      for (const scenario of scenarioDefs) {
        await s.test(`SCENARIO ${scenario.id} — ${scenario.name}`, async (ss) => {
          const product = getProductByScenarioId(scenario.id);
    ...
    await t.test('SECTION 3 — Product Grid Review', async (s) => {
      ...
      for (const scenario of sampleScenarios) {
        await s.test(`GRID — ${scenario.name}`, async () => {
          const product = getProductByScenarioId(scenario.id);
          const payload = await getReviewPayload(product.productId);
    ...
    await t.test('SECTION 4 — Component Review', async (s) => {
      for (const ct of contractAnalysis._raw.componentTypes) {
        await s.test(`COMP — ${ct.type}`, async () => {
          ...
          // GAP-4: pipeline-discovered components should have new_component flag
          const pipelineRows = result.items.filter(r =>
            r.name_tracked?.reason_codes?.includes('new_component'));
          assert.ok(pipelineRows.length > 0,
            `${ct.type} should have at least 1 row with new_component flag ` +
            `(pipeline-discovered). name_tracked sources: [${result.items.map(r => r.name_tracked?.source).join(', ')}]`);

          // GAP-7: variance_violation on property slots (when variance policies exist)
          if (Object.keys(ct.allVariancePolicies).length > 0) {
            const allPropertyReasonCodes = result.items.flatMap(r =>
              Object.values(r.properties || {}).flatMap(p => p.reason_codes || [])
            );
            const hasVarianceViolation = allPropertyReasonCodes.includes('variance_violation');
            assert.ok(hasVarianceViolation,
              `${ct.type} should have at least 1 property slot with variance_violation flag ` +
              `(from variance_policies scenario). all property reason_codes: [${[...new Set(allPropertyReasonCodes)].join(', ')}]`);
          }
        });
      }
    });
    ...
    await t.test('SECTION 6 — Coverage Matrix Verification', async (s) => {
      ...
      await s.test('MATRIX-11 — multi_source_consensus sources arrive in non-tier-sorted order', () => {
        const consensusProduct = getProductByScenarioName('multi_source_consensus');
    ...
    await t.test('SECTION 8 — Flag Cleanup', async (s) => {
      await s.test('FLAG-01 — happy_path metrics.flags is 0', async () => {
        ...
        const payload = await getReviewPayload(product.productId, { withSpecDb: true });
      ...
      await s.test('FLAG-03 — preserve_all_candidates product has conflict_policy_hold flag', async () => {
        const product = getProductByScenarioName('preserve_all_candidates');
        if (!product) return;
        const payload = await getReviewPayload(product.productId, { withSpecDb: true });
      ...
      await s.test('FLAG-05 — cross_validation product has constraint_conflict flag (GAP-6)', async () => {
        const product = getProductByScenarioName('cross_validation');
        if (!product) return;
        const payload = await getReviewPayload(product.productId);
        const flaggedFields = Object.entries(payload.fields)
          .filter(([, state]) => (state.reason_codes || []).includes('constraint_conflict')
            || (state.reason_codes || []).includes('compound_range_conflict'));
        const rules = contractAnalysis._raw.rules || [];
        const triggerFields = [...new Set(rules.flatMap(r =>
          [r.trigger_field, ...(r.related_fields || []), ...(r.depends_on || [])].filter(Boolean)
        ))];
        assert.ok(flaggedFields.length > 0,
          `cross_validation product should have at least 1 field with constraint_conflict flag, ` +
          `trigger fields: [${triggerFields.join(', ')}], ` +
          `all reason_codes: ${JSON.stringify(Object.fromEntries(
            Object.entries(payload.fields).filter(([, s]) => s.reason_codes?.length > 0).map(([k, s]) => [k, s.reason_codes])
          ))}`);
      });
