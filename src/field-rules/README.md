...
- `fieldRuleCapabilities.js`: `FIELD_RULE_CAPABILITIES`, `FIELD_RULE_CAPABILITY_KEYS`, `capabilityKeyForSchemaPath(path)` — derived no-dead-config registry. Schema knobs derive from `FIELD_RULE_SCHEMA`; compatibility-only knobs live in one local legacy block.
