...
export async function syncCategorySchemaFromCatalog({
  category,
  config = {}
}) {
  const fieldRulesLoaded = await loadGeneratedFieldRules(category, config);
  ...
  if (!extracted.enabled) {
    return {
      category,
      synced: false,
      reason: extracted.error || 'catalog_extract_failed',
