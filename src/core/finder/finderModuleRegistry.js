  ...
  },
  {
    ...
    settingsSchema: [
      ...
      { key: 'budgetVariantPointsPerExtra', type: 'float', default: 0.25, min: 0, max: 10,
        ...
        uiTip: 'Points added to the per-key attempt budget for each variant beyond the first. Fractional so variant count is a gentle accrual, not a whole-point punch per variant. Raw budget accrues the fractional value; final attempts = ceil(raw).' },
      ...
      { key: 'bundlingPassengerVariantCostPerExtra', type: 'float', default: 0.25, min: 0, max: 10,
        uiLabel: 'Passenger variant cost per extra', uiGroup: 'Bundling',
        uiTip: 'Additional passenger-cost points added for each family variant beyond the first. Example: easy base cost 1 plus 0.25 means family size 2 costs 1.25, size 3 costs 1.5, size 4 costs 1.75.' },
